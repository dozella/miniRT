...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/04 16:27:53 by cnearing          #+#    #+#             */
/*   Updated: 2022/11/04 16:27:54 by cnearing         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	light_inter(t_disp *disp, t_light *l, t_pix pix)
{
	...
	while (obj)
	{
		...
		if (obj->type == SPH && sph_inter(&rl, (t_sph *)obj->obj)
			...
			&& rl.pix.t * rl.pix.t < get_norm2(op_subs(l->o, pix.pt)))
			return (1);
		else if (obj->type == CO && co_inter(&rl, (t_co *)obj->obj)
			&& rl.pix.t * rl.pix.t < get_norm2(op_subs(l->o, pix.pt)))
			return (1);
		obj = obj->next;
	}
...
int	color_def(t_disp *disp, t_ray *rc)
{
	double	wpix;
	t_lobj	*light;
	t_light	*l;
	...
	{
		l = (t_light *)light->obj;
		if (!light_inter(disp, l, rc->pix))
			wpix = (fmax(0., l->w * dot(get_normalize(\
				op_subs(l->o, rc->pix.pt)), rc->pix.norm)) + phong(l, rc))
				/ powf(get_norm2(op_subs(l->o, rc->pix.pt)), 0.25);
		else
			wpix = 0;
		color[0] = wpix * (double)l->color[0] + color[0];
		color[1] = wpix * (double)l->color[1] + color[1];
		color[2] = wpix * (double)l->color[2] + color[2];
		light = light->next;
	}
	color[0] = fmin(255., color[0] * (double)rc->pix.color[0] / 255.);
	color[1] = fmin(255., color[1] * (double)rc->pix.color[1] / 255.);
	color[2] = fmin(255., color[2] * (double)rc->pix.color[2] / 255.);
...
void	objs_inter(t_disp *disp, t_ray *rc, int x, int y)
{
	...
	while (obj)
	{
		if (obj->type == SPH)
			...
			co_inter(rc, (t_co *)obj->obj);
		obj = obj->next;
	}
	if (rc->pix.active)
		pix_actualisation_img(disp, x, -y, color_def(disp, rc));
}
